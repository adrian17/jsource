...
F1(jtparsea){A *stack,*queue,y,z,*v;I es,i,m,otop=jt->nvrtop,maxnvrlen,*dci=&jt->sitop->dci; B jtxdefn=jt->xdefn;
 ...
 // It's a pity that this needs to be set BEFORE executing the action routine - otherwise we could set it
 // only on failure - but the error message is formatted at the time of error, in jsignal()
...
#define ST(i) AT(stack[2*i])
   ...
   *dci = (I)stack[(i&6)|1];  // set the token-in-error, using the offset encoded into i
   ...
   if(m<=0) {  // Toward the end we have to worry about underrunning the queue, and pulling the virtual mark
     if(m==0) {stack -= 2; stack[0] = mark; --m; continue;}  // unstructured branch!! move in the mark and use it.  Token number never used for mark
      ...
      *dci = m;  // syrd can fail, so we have to set the error-word number before calling it
      if(s=syrd(y,0L)) {   // look up the name in the symbol tables.  0L=Don't bother storing which symbol table was used
        ...
        if(!(sv = s->val))FP  // symbol table entry, but no value.
          // Following the original parser, we assume this has been detected earlier.  No ASSERT here, since we must pop vr stack
        ...
        if(NMDOT&NAV(y)->flag&&jtxdefn)FP  // Musn't ASSERT: need to pop nvr stack
...
exitparse:
 ...
 ASSERT(AT(z)&MARK+CAVN&&AT(stack[4])==MARK,(*dci = 0,EVSYNTAX));  // OK if 0 or 1 words left (0 should not occur)
 R z;
}
